...
public class EmployeeDetails extends JFrame implements ActionListener, ItemListener, DocumentListener, WindowListener {
    ...
    private JPanel navigPanel() {
        ...
        first = new NavigateButtonBuilder("first.png")
                .setPreferredSize(new Dimension(17, 17))
                .addActionListener(this)
                .setToolTipText("Display first Record")
                .build();
        navigPanel.add(first);

        previous = new NavigateButtonBuilder("prev.png")
                .setPreferredSize(new Dimension(17, 17))
                .addActionListener(this)
                .setToolTipText("Display prev Record")
                .build();
        navigPanel.add(previous);

        next = new NavigateButtonBuilder("next.png")
                .setPreferredSize(new Dimension(17, 17))
                .addActionListener(this)
                .setToolTipText("Display next Record")
                .build();
        navigPanel.add(next);

        last = new NavigateButtonBuilder("last.png")
                .setPreferredSize(new Dimension(17, 17))
                .addActionListener(this)
                .setToolTipText("Display last Record")
                .build();
        navigPanel.add(last);
    ...
    private void displayEmployeeSummaryDialog() {
        ...
        if (EmployeeUtils.isSomeoneToDisplay(this))
    ...
    private void firstRecord() {
        new FirstRecord(this).retrieveRecord();
    ...
    private void previousRecord() {
        new PreviousRecord(this).retrieveRecord();
    }// end previousRecord

    // find byte start in file for next active record
    private void nextRecord() {
        new NextRecord(this).retrieveRecord();
    }// end nextRecord

    // find byte start in file for last active record
    private void lastRecord() {
        new LastRecord(this).retrieveRecord();
    ...
    public int getNextFreeId() {
        ...
        if (file.length() == 0 || !EmployeeUtils.isSomeoneToDisplay(this))
            nextFreeId++;
        else {
    ...
    private void deleteRecord() {
        if (EmployeeUtils.isSomeoneToDisplay(this)) {// if any active record in file display
